"""Model-variant configuration for the deployment app.

The deployed app supports one framework: EG-PCS-Net with DINOv3 and the
attention-alignment model variant used during training. At inference time uploaded
...
def build_model_variant_config(args, *, is_cnn_backbone: bool, out_size: int | None = None) -> ModelVariantConfig:
    """Return the only model variant supported by this deployment app.

    The parameters are kept for compatibility with the training metadata loader,
    but this app intentionally supports only EG-PCS-Net with attention alignment.
    """
    del out_size
    if is_cnn_backbone:
        raise ValueError("This deployment app only supports the DINOv3 transformer backbone.")

    args.model_variant = "EG-PCS-Net"
